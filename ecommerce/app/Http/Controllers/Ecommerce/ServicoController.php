...
class ServicoController extends Controller
{
    ...
    public function view($destino_slug = null, $categoria_slug = null, $servico_slug = null)
    {
        ...
        if ($servico->status == 'INATIVO') return redirect()->route('ecommerce.index');
        ...
        // if ($servico_slug == 'passaporte-da-diversao-vip-5-atracoes') {
        //     return redirect($destino_slug . '/' . $categoria_slug . '/passaporte-da-diversao-vip-4-atracoes-oceanic-aquarium-aventura-pirataclassic-car-show-e-aventura-jurassica', 301);
        // }

        // if ($servico_slug == 'tour-uva-e-vinho-com-trem-maria-fumaca') {
        //     return redirect($destino_slug . '/' . $categoria_slug . '/tour-uva-e-vinho-com-trem-maria-fumaca-duas-vinicolas', 301);
        // }
    ...
    public function avaliacoes(Request $request, $uuid)
    {
        // Busca o serviço
        $servico = Servico::where('uuid', $uuid)->get()->first();

        // Caso o serviço não seja encontrado. Retorna um 404
        if($servico == null) return response(['info' => 'Serviço não encontrado', 'status' => false], 404);

        // A partir de qual registro será retornado.
        // Serve para evitar que enviemos a mesma avaliação que o cliente ja esta vendo no site
        $inicial = $request->get('inicial', 0);

        // Qual o limite que será retornado
        $limite = $request->get('limite', 100);

        // Avaliações
        $avaliacoes = AvaliacaoService::getAvaliacoesAprovadas($servico, $limite);

        // Remove as primeiras usando a variavel $inicial para garantir que mandemos avaliações que o cliente ja viu
        for($i = 0; $i < $inicial; $i++) {
            $avaliacoes->shift($i);
        }

       foreach($avaliacoes as $key => $avaliacao) {
            $avaliacoes[$key]['data'] = 'Feito em ' . $avaliacao->created_at->format('d/m/Y');
       }

        // Retorna para o cliente
        return response(['status' => true, 'avaliacoes' => $avaliacoes], 200);
    }
}
