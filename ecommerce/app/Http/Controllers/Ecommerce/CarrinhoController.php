...
class CarrinhoController extends Controller
{
    ...
    public function index()
    {
        ...
        $servicos_indisponiveis = ConferenciaReserva::validarCarrinho();


        $texto_indisponiveis = "";
        if($servicos_indisponiveis['problemas'] > 0) {
            foreach($servicos_indisponiveis['servicos'] as $servico) {
                $texto_indisponiveis .= "{$servico['servico']->nome}, ";
            }
            $texto_indisponiveis = substr($texto_indisponiveis, 0, -2);
        }
        ...
        $dados = [
            ...
            'perguntas' => $perguntas,
            'expirado' => $expirado,
            'servicos_indisponiveis' => $servicos_indisponiveis,
            'texto_indisponiveis' => $texto_indisponiveis,
        ];
    ...
    public function faq()
    {
        return [
            ...
            [
                ...
                'resposta' => 'Você pode pagar suas compras com cartão de crédito à vista ou parcelado com as seguintes bandeiras: Visa, MasterCard, Hipercard, Elo e Amex. Para mais formas de pagamentos, entre em contato conosco via e-mail ou Whatsapp.',
            ]
        ];
    }
    ...
    public function linkVenda(Request $request, $uuid) {
        ...
        if($venda_interna->vendedor_id != null) {
            $vendedor = Vendedor::find($venda_interna->vendedor_id);
