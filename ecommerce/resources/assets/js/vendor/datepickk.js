...
function Datepickk(args) {
	...
	function setDate() {
		...
		if (maxDate && new Date(currentYear, currentMonth - 1 + months - 1, 1) >= new Date(maxDate).setDate(1)) {
			...
			that.el.header.childNodes[2].classList.add('picker-disable');
		} else {
            that.el.header.childNodes[2].classList.remove('picker-disable');
		}
		if (minDate && new Date(currentYear, currentMonth - 1, 1) <= new Date(minDate).setDate(1)) {
			currentYear = minDate.getFullYear();
			currentMonth = minDate.getMonth() + 1;
			that.el.header.childNodes[0].classList.add('picker-disable');
		} else {
			that.el.header.childNodes[0].classList.remove('picker-disable');
		}
