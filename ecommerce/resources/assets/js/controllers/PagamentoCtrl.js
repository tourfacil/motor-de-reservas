let PagamentoCtrl = {
    ...
    init: (page) => {
        ...
        // Click para gerar de fato o código
        PagamentoCtrl.gerarCodigoPix();
    ...
    /**
        Responsavel por gerar um código PIX quando o usuário clica no botão de "GERAR CÓDIGO PIX"
     */
    gerarCodigoPix: () => {
        $("#btn_gerar_codigo_pix").on('click', (event) => {

            let option = $this.find('option:selected');
            let option_pix = 'pix';
            let option_cartao_credito = 'cartao_credito';
            let servico_nome = $("input#servico_nome").val();
            let servico_valor = $("input#servico_valor").val();

            if(PagamentoCtrl.isCamposValidosParaPix() == false) {
                return;
            }
            ...
            $("select#forma_pagamento").prop('disabled',true);
            $("#alterar_forma_pagamento").show();

            let url_post = PagamentoCtrl._urlGeracaoQrcodePagamentoPix;

            let req = axios.post(url_post, cliente).then((response) => {
                let data = response.data;

                if(data.status == false) {
                    Helpers.loader.hide();
                    return swal('Erro', 'Houve um erro ao gerar o código PIX. Atualize a página e tente novamente', 'warning').then(() => {
