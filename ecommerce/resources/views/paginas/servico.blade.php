...
@section('title', getTituloPaginaServico($servico))
            ...
            @foreach ($servico->fotos as $foto_servico)
                ...
                <div class="item">
                    <a href="{{ $foto }}" title="Imagem {{ $foto_servico->legenda }}" data-lightbox="galeria">
                        <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" class="owl-lazy"
                            data-src="{{ $foto }}" alt="{{ $foto_servico->legenda }}">
                    </a>
                </div>
            @endforeach
        </div>
        <button data-action="openGallery" class="btn btn-purple btn-rounded px-4 pb-2 border-0 d-none d-lg-inline">Ver
            galeria completa</button>
    </div>

    <main class="container" data-page="servico" data-controller="ServicoCtrl"
        data-servico="{{ route('ecommerce.servico.view.json', [$servico->uuid, 'category' => $categoria->slug]) }}">
        {{-- breadcrumb --}}
        <nav class="custom-bread pt-3 pb-2" aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item first-mobile"><a
                        href="{{ route('ecommerce.destino.index', $destino->slug) }}">{{ $destino->nome }}</a></li>
                <li class="breadcrumb-item"><a
                        href="{{ route('ecommerce.categoria.index', [$destino->slug, $categoria->slug]) }}">{{ $categoria->nome }}</a>
                </li>
                ...
                <div class="fixed-content">
                    ...
                    <div class="rounded bg-grey p-4 card-buy ml-lg-2 mb-3 mb-md-0 mt-2 mt-lg-0">
                        <div class="text-center">
                            ...
                            <div class="d-flex align-items-start justify-content-center card-buy-valor">
                                ...
                                @else
                                    <div class="trf" style="display: flex; flex-direction: column;">
                                        <span style="text-decoration:line-through;">De R$
                                            {{ formataValor($servico->descontoAtivo->valor_de) }}</span>
                                        <strong class="price"
                                            style="color: #e73677;">R${{ formataValor($servico->descontoAtivo->valor_por) }}</strong>
                                    </div>
                                @endif
                            </div>
                            <div class="card-buy-valor mt-3">
                                <strong class="h5 d-block text-muted mb-3">Parcele em até
                                    {{ config('site.parcelamento') }}x!</strong>
                                    <div class="desconto-pix-servico text-center mt-3 p-2">
                                        <p class="h5 mb-0" style="color: #4e4e4e;">Pague apenas R$ {{ formataValor($servico->valor_venda * (1 - (\TourFacil\Core\Services\Pagamento\DescontoPIXService::getPixDesconto() / 100))) }} no Pix</p>
                                    </div>
                            ...
                            <div class="text-center mt-3">
                                <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs="
                                    class="lazyload img-fluid" data-src="{{ asset('images/bandeiras_cartao.svg') }}"
                                    alt="Cartões aceitos">
                                <button class="btn btn-link btn-not-focus" title="Veja mais opções de pagamento"
                                    data-modal="#modal-formas-pagamento">Ver mais opções de pagamento</button>
                            </div>
                        </div>
                        <div class="text-center mt-3 d-none d-block">
                            @if ($ativo)
                                <style>
                                    button[data-action="comprar"]:hover {
                                ...
                                <button data-action="comprar"
                                    class="btn btn-purple btn-not-focus btn-rounded text-uppercase font-weight-medium">COMPRAR
                                    AGORA</button>
                            @else
                    ...
                    <div class="faq-custom ml-lg-2 mt-3 d-none d-lg-block">
                        ...
                        <div class="accordion" id="accordionExample">
                            @if ($servico->faqServico->count() == 0)
                                @foreach ($perguntas as $index => $pergunta)
                                    <div class="accordion-item" id="faq_{{ $index }}">
                                        <button class="btn btn-link d-flex align-items-center" type="button"
                                            data-toggle="collapse" data-target="#res_{{ $index }}">
                                            <i class="iconify mt-1 mr-2" data-icon="jam:chevron-down"></i>
                                            {{ $pergunta['questao'] }}
                                        </button>
                                        <div id="res_{{ $index }}" class="accordion-body collapse"
                                            data-parent="#faq_{{ $index }}">
                                            <p class="pl-4 py-2 rounded bg-light m-0">{!! $pergunta['resposta'] !!}</p>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                @foreach ($servico->faqServico as $index => $pergunta)
                                    <div class="accordion-item" id="faq_{{ $index }}">
                                        <button class="btn btn-link d-flex align-items-center" type="button"
                                            data-toggle="collapse" data-target="#res_{{ $index }}">
                                            <i class="iconify mt-1 mr-2" data-icon="jam:chevron-down"></i>
                                        ...
                                        <div id="res_{{ $index }}" class="accordion-body collapse"
                                            data-parent="#faq_{{ $index }}">
                                            <p class="pl-4 py-2 rounded bg-light m-0" style="text-align: justify;">
                                                {!! $pergunta->texto !!}</p>
                                        </div>
                                    </div>
                                @endforeach
                ...
                <section class="service-details pr-md-1 mt-1 mt-md-3 mt-lg-0">
                    @if ($servico->tagsInternas->count() > 0)
                        <div style="margin-bottom: 15px;">
                            ...
                            <div
                                style="background-color: rgba(0, 0, 0, 0.017);padding: 0.8rem 1rem; border-radius: 3px; margin: 0.8rem 0 0.95rem 0; text-align: justify;">
                                @foreach ($servico->tagsInternas as $tag)
                                    <div style="margin-bottom: 15px;">
                                        <div style="margin-bottom: 10px;">
                                            <h4 style=" color: #4E4E4E;">
                                                <i style="margin-right: 5px;" class="{{ $tag->icone }}"></i>
                                                {{ $tag->titulo }}
                                            </h4>
                                        </div>
                    ...
                    @if (!is_null($servico->localizacao))
                        ...
                        <div class="localizacao mb-2">
                            <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs="
                                class="lazyload img-fluid" data-src="{{ asset('images/icons/icone-mapa.svg') }}"
                                alt="Icone como chegar">
                            <p>{{ $servico->localizacao }}</p>
                        </div>
                        <div class="embed-responsive rounded mb-2 mt-4">
                            <iframe data-hj-allow-iframe class="embed-responsive-item lazyload rounded"
                                data-src="https://www.google.com/maps/embed/v1/place?key={{ env('GOOGLE_API_KEY') }}&q={{ Str::slug($servico->localizacao, '+') }}+{{ Str::slug($servico->fornecedor->nome_fantasia, '+') }}"
                        ...
                        <div class="text-center mt-4 d-md-none">
                            <a href="https://www.google.com/maps?daddr={{ Str::slug($servico->localizacao, '+') }}+{{ Str::slug($servico->fornecedor->nome_fantasia, '+') }}&hl=pt-br"
                                target="_blank" title="Ver no mapa a localização"
                                class="btn btn-blue btn-rounded text-white px-4 pb-2">Ver no mapa</a>
                        </div>
                    @endif

                    @if ($total_avaliacoes > 0)
                        <h3>Avaliações <small style="margin-left: 10px;">{{ $nota_media }}</small> <img
                                style="margin-bottom: 5px;" src="{{ asset('images/star1-24.png') }}">
                            <small>({{ $total_avaliacoes }} avaliações)</small>
                        </h3>
                        <div id="section-avaliacoes">
                            @foreach ($avaliacoes as $key => $avaliacao)
                                <div class="avaliacaoi-{{ $key }}">
                                    ...
                                    @else
                                        <p id="avaliacao-resumo-{{ $key }}"
                                            style="text-align: justify; margin-bottom: 0px;">
                                            {{ substr($avaliacao->avaliacao, 0, 280) }}...</p>
                                        <p id="avaliacao-completo-{{ $key }}"
                                            style="text-align: justify; display: none;">{{ $avaliacao->avaliacao }}</p>
                                        <a id="ver-mais-avaliacao-{{ $key }}" key="{{ $key }}"
                                            class="btn-ver-mais" href="#">Ver mais</a>
                                        <a style="display: none;" id="ver-menos-avaliacao-{{ $key }}"
                                            key="{{ $key }}" class="btn-ver-menos" href="#">Ver menos</a>
                                    @endif
                        ...
                        @if ($total_avaliacoes > 2)
                            <button id="carregar-mais-avaliacoes" class="btn btn-purple-outline btn-rounded px-4">Ver mais
                                avaliações</button>
                            <div style="display: none;">
                                <span id="avaliacao-qtd">{{ $total_avaliacoes }}</span>
                                <span id="avaliacao-qtd-mostrada">2</span>
                                <span
                                    id="avaliacao-link">{{ Route('ecommerce.servico.avaliacoes', $servico->uuid) }}</span>
        ...
        <div class="fixed-bottom-mobile d-lg-none px-3 py-2">
            <div class="row flex-nowrap justify-content-between align-items-center">
                ...
                <div class="col-auto pl-0">
                    @if ($ativo)
                        <button data-action="comprar"
                            class="btn btn-purple btn-not-focus btn-rounded border-0 px-md-4 pb-2 text-uppercase font-weight-medium">COMPRAR
                            AGORA</button>
                    @else
                ...
                <div class="owl-carousel owl-nav-blue owl-nav-container owl-relacionados mt-4 mb-5 owl-padding">
                    @foreach ($relacionados as $servico_relacionado)
                        ...
                        <div class="card-servico item">
                            <a href="{{ route('ecommerce.servico.view', [$destino->slug, $categoria->slug, $servico_relacionado->slug]) }}"
                                title="{{ $servico_relacionado->nome }}">
                                <div class="card-servico-image">
                                    <img src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs="
                                        class="{{ $lazy_class }}"
                                        data-src="{{ $servico_relacionado->fotoPrincipal->fotoLarge }}"
                                        alt="{{ $servico_relacionado->nome }}">
                                </div>
