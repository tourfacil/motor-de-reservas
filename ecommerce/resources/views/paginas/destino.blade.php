...
@if($destino->titulo_pagina == null)
    @section('title', "Os Melhores Ingressos em {$destino->nome}  - Tour Fácil")
@else
    @section('title', $destino->titulo_pagina)
@endif
...
@section('body')
    ...
    <main data-page="destino" class="pb-4 mb-3">
        ...
        {{-- CAMPANHA --}}
        @IF(config('campanha.ativo') == true && $destino->id == 1)
            ...
            <section class="section-servicos-populares mt-4 mb-4" id="namorados" class="namorados">
                <div class="container pt-3">
                    <h2 class="text-center font-weight-medium pb-2" style="color: {{ config('campanha.cor_texto_container') }}">{{ config('campanha.titulo') }}</h2>
                                ...
                                <a href="{{ route('ecommerce.servico.view', [$destino->slug, $servico_home->categoria->slug, $servico_home->slug]) }}" title="{{ $servico_home->nome }}">
                                    ...
                                    <div class="card-body d-flex flex-column">
                                        ...
                                        <div class="mt-auto d-flex justify-content-end align-items-center card-body-price">
                                            ...
                                            @if($servico_home->descontoAtivo == false)
                                                <span>A partir de</span>
                                                <strong>R$ {{ formataValor($servico_home->valor_venda) }}</strong>
                                            @else
                                                <div style="display: flex !important; flex-direction: row; flex-wrap: wrap; justify-content: flex-end; align-items: flex-start;">
                                                    <span style="text-decoration: line-through;">De R$ {{ formataValor($servico_home->descontoAtivo->valor_de) }}</span>
                                                    <strong style="color: #e73677;">Por R$ {{ formataValor($servico_home->descontoAtivo->valor_por) }}</strong>
                                                </div>
                                            @endif
