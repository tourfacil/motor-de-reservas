        ...
        <div class="row justify-content-between">
            ...
            <div class="col-12 col-lg-8 pr-lg-4">
                ...
                <form data-validate-post class="mt-1 mt-md-2" action="{{ route('ecommerce.carrinho.efetuar-pagamento') }}" method="POST">
                    ...
                    <hr class="blue">
                    <div class="row mb-3">
                        @if(\TourFacil\Core\Services\Pagamento\Gerencianet\Pix\PixService::isPixEnabled())
                            <div class="col-12 col-md-12 float-left" style="display: {{ isPixAtivo() ? "none" : "block" }};">
                        ...
                        <div class="float-right col-12 col-md-12 mt-4" style="display: none" id="btn_gerar_codigo_pix" >
                            <a title="Ver mais atividades" class="btn btn-blue-outline btn-block btn-rounded pb-2 text-uppercase">Gerar código PIX</a>
                        </div>
                        <div class="float-right col-12 col-md-12 mt-4" style="display: {{ isPixAtivo() ? "block" : "none" }};">
                            <a id="alterar_forma_pagamento" title="Ver mais atividades" class="btn btn-blue-outline btn-block btn-rounded pb-2 text-uppercase">Alterar Forma de Pagamento</a>
                        </div>
                    </div>

                    {{-- Crédito --}}
                    <div id="cartao_credito" style="display: {{ \TourFacil\Core\Services\Pagamento\Gerencianet\Pix\PixService::isPixEnabled() ? 'none' : 'block' }}">
                        ...
                        <div class="mt-3 pt-1 row d-flex align-items-center justify-content-center">
                            <div class="col-12 col-md-6">
                                ...
                                <select id="numero_parcelas" tabindex="{{ $index_input++ }}" name="credito[parcelas]"
                                    class="custom-select" data-required required title="Número de parcelas">
                        ...
                        <div class="d-none">
                            ...
                            <input type="hidden" name="metodo_pagamento" value="{{ $e_pagamento_cartao }}">
                        </div>
                        <div class="text-center">
                ...
                {{-- Pix --}}
                <div id="pix" style="display: {{ isPixAtivo() ? "block" : "none" }}; text-align: center">
                    <div style="width: auto;">
                        <img id="qrcode_pix" src="data:image/png;base64,{{ session()->get('pix.qrcode_image') }}">
                    </div>
                    <div style="width: auto">
                        <hr style="width: 100%; float: center">
                    </div>
                    <fieldset style="width: 100%; border: 1px solid black; opacity: 0.65; border-radius: 5px; padding: 10px">
                        <legend style="width: initial; font-size: 16px;">&nbsp;Copiar código Pix&nbsp;</legend>
                        <div>
                            <p id="codigo_pix_exibido" style="width: 100%; word-break: break-all;">{{ session()->get('pix.codigo_pix') }}</p>
                            <p id="codigo_pix" style="display: none">{{ session()->get('pix.codigo_pix') }}</p>
                        </div>
...
<script>
    ...
    @if(isPixAtivo())
        ...
        let verificarPix = setInterval(() => {

            if(PagamentoCtrl.isCamposValidosParaPix()) {
                let link = "{{ route('ecommerce.carrinho.consultar-pagamento-pix') }}";
                console.log(link);
                ...
                axios.post(link, {}).then((response) => {
                    ...
                    if(data.pagamento_realizado == true) {

                        clearInterval(verificarPix);

                        PagamentoCtrl.requestGerarPedido();
                    }
                })
            }
        }, 5000);
