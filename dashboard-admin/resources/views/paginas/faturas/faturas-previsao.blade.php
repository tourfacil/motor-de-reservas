                        ...
                        <div class="ml-2">
                            <div class="row">
                                ...
                                <div class="input-group">
                                    <input id="data_venda" type="tel" class="form-control datepicker"
                                        placeholder="DD/MM/AAAA" data-required title="Data inicial"
                                        value="{{ $inicio->format('d/m/Y') . ' - ' . $final->format('d/m/Y') }}">
                                </div>
                                ...
                                @foreach ($faturas as $fatura)
                                    <tr style="color: #5d5386;">
                                        ...
                                        <td>
                                            ...
                                            <span
                                                class="badge-text badge-text-small {{ \TourFacil\Core\Enum\Faturas\StatusFaturaEnum::CORES[$fatura->status] }}">
                                                Previsão de fatura
                                            </span>
                                        <td>{{ $fatura->tipo }}</td>
                                        ...
                                        <td>
                                            <a href="{{ Route('app.faturas.fatura-prevista-individual') . '?fornecedor=' . $fatura->fornecedor_id . '&inicio=' . $fatura->inicio->format('Y-m-d') . '&final=' . $fatura->final->format('Y-m-d') }}"
    ...
    <script>
            ...
            Plugins.loadLibDateRanger(() => {
                ...
                let config = {
                    ...
                    locale: {
                        ...
                        monthNames: ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out",
                            "Nov", "Dez"
                        ],
                    }
                };
        ...
        let onCloseDataVenda = () => {
            $("input#data_venda").on('apply.daterangepicker', (event1, event2) => {
                ...
                data_inicial_venda =
                    `${date_inicio.getFullYear()}-${date_inicio.getMonth() + 1}-${date_inicio.getDate()}`;
                data_final_venda =
                    `${date_final.getFullYear()}-${date_final.getMonth() + 1}-${date_final.getDate()}`;
