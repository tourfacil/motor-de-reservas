        ...
        <div class="row d-flex align-items-center text-centers">
            <div class="col-4">
                @if($data_inicial_venda && $data_final_venda)
                    ...
                    <h4 class="font-weight-bold">{{ $data_inicial_venda->format('d/m/Y') }} até {{ $data_final_venda->format('d/m/Y') }}</h4>
                @endif
                @if($data_inicial_utilizacao && $data_final_utilizacao)
                    <p class="mt-0 mb-2">Período de utilização</p>
                    <h4 class="font-weight-bold">{{ $data_inicial_utilizacao->format('d/m/Y') }} até {{ $data_final_utilizacao->format('d/m/Y') }}</h4>
                @endif
            </div>
            <div class="col-2">
                <p class="mt-0 mb-2">Total vendido</p>
                <h4 class="font-weight-bold">R$ {{ formataValor($valor_total_venda ?? 0) }}</h4>
            </div>
            <div class="col-2">
                <p class="mt-0 mb-2">Total NET</p>
                <h4 class="font-weight-bold">R$ {{ formataValor($valor_total_net) }}</h4>
            </div>
