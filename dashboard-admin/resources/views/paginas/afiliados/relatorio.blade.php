...
<div class="row">
    <div class="page-header">
        <div class="d-flex align-items-center">
            ...
            <div>{{ Breadcrumbs::render('app.afiliados.relatorios.index') }}</div>
        </div>
    </div>
</div>
                    ...
                    <div class="row d-flex align-items-center">
                        ...
                        <div class="col-4 text-right">
                            <div class="form-group pl-2 m-0">
                                @IF($afiliado != null)
                                    <a href="{{ route('app.relatorios.afiliados.download.pdf') }}?afiliado_id={{$afiliado->id}}&inicio={{$data_inicio->format('d-m-Y')}}&final={{$data_final->format('d-m-Y')}}&tipo_operacao={{$tipo_operacao}}&comissao_passeios={{$afiliado->comissao_passeios}}&comissao_ingressos={{$afiliado->comissao_ingressos}}&comissao_gastronomia={{$afiliado->comissao_gastronomia}}&comissao_transfer={{$afiliado->comissao_transfer}}" target="_blank"
                                        class="btn btn-secondary mr-2 line-height-inherit">Baixar PDF <i class="la la-download right"></i></a>
                                @ENDIF
                                @IF($afiliado != null)
                                    <a href="{{ route('app.relatorios.afiliados.download.xls') }}?afiliado_id={{$afiliado->id}}&inicio={{$data_inicio->format('d-m-Y')}}&final={{$data_final->format('d-m-Y')}}&tipo_operacao={{$tipo_operacao}}&comissao_passeios={{$afiliado->comissao_passeios}}&comissao_ingressos={{$afiliado->comissao_ingressos}}&comissao_gastronomia={{$afiliado->comissao_gastronomia}}&comissao_transfer={{$afiliado->comissao_transfer}}" target="_blank"
                                    ...
                                    <tr>
                                        ...
                                        <td  style="color: #5d5386;">
                                            <a class="text-truncate text-center">
                                                {{ \TourFacil\Core\Services\AfiliadoService::getComissaoPercentual($reserva) }}
                                            </a>
                                        </td>
                                        <td  style="color: #5d5386;">
                                            <a class="text-truncate">
                                                R$ {{ formataValor(\TourFacil\Core\Services\AfiliadoService::getComissaoAfiliado($reserva)) }}
                                            </a>
                                        </td>
                                    </tr>
                                @ENDFOREACH
