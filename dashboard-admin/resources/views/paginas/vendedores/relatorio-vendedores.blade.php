    ...
    <div class="row">
        <div class="page-header">
            <div class="d-flex align-items-center">
                <h2 class="page-header-title text-dark">Vendedores <span class="text-gradient-01">que possuem vendas</span> </h2>
                    ...
                    <div class="row d-flex align-items-center">
                        <div class="col-7">
                            <div class="form-group pl-2 m-0">
                                {{-- <a href="{{ Route('app.relatorios.vendedores.index2-pdf') }}?tipo_operacao={{ $tipo_data }}&periodo=custom&inicio={{ $inicio }}&final={{ $final }}" target="_blank"
                                   class="btn btn-secondary mr-2 line-height-inherit">Baixar PDF <i class="la la-download right"></i></a>
                                <a href="{{ Route('app.relatorios.vendedores.index2-xls') }}?tipo_operacao={{ $tipo_data }}&periodo=custom&inicio={{ $inicio }}&final={{ $final }}" target="_blank"
                ...
                <div class="widget-body pr-2 pl-2">
                    <div class="table-responsive">
                        <table class="table table-hover table-border-bottom mb-0 data-table" data-page-length="1000">
                            ...
                            <tbody>
                            @forelse($vendedores as $nome => $vendedor)
                                <tr>
                                    <td width="10" class="text-center pt-3 pb-3">{{ $loop->iteration }}</td>
                                    <td>
                                        <div class="d-inline text-truncate m-0">
                                            <a href="{{ Route('app.relatorios.vendedores.index') }}?vendedor_id={{ $vendedor['vendedor']['id'] }}&tipo_operacao={{ $tipo_data }}&periodo=custom&inicio={{ $inicio }}&final={{ $final }}" title="Ver fornecedor" class="text-truncate" target="_blank">
                                                {{ $nome }}
                                            </a>
                                        </div>
                                    </td>
                                    <td>{{ $vendedor['vendedor']['cnpj'] }}</td>
                                    <td data-order="{{ $vendedor['valor_venda'] }}" class="text-success text-center font-weight-bold">R$ {{ formataValor($vendedor['valor_venda']) }}</td>
                                    <td data-order="{{ $vendedor['valor_comissao'] }}" class="text-warning text-center font-weight-bold">R$ {{ formataValor($vendedor['valor_comissao']) }}</td>
                                    <td data-order="{{ $vendedor['quantidade'] }}" class="text-info text-center font-weight-bold">{{ $vendedor['quantidade'] }} ingresso(s)</td>
                                    <td class="text-center">
                                        <a href="{{ Route('app.relatorios.vendedores.index') }}?vendedor_id={{ $vendedor['vendedor']['id'] }}&tipo_operacao={{ $tipo_data }}&periodo=custom&inicio={{ $inicio }}&final={{ $final }}" target="_blank" title="Ver relatório" class="btn btn-outline-primary">
