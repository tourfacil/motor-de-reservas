let DadosFactory = {
    ...
    setItem: (key, data, email) => {
        ...
        localStorage.setItem(final_key, JSON.stringify(data));
    },
     ...
     * @param new_acomp
     */
    saveDadosAcompanhantes: (new_acomp, email) => {
        // Transforma em array
        ...
        // Salva os dados
        DadosFactory.setItem(DadosFactory._indexAcompanhantes, acompanhantes, email);
    },
     ...
     * @returns {*|any[]}
     */
    getDadosAcompanhantes: (email) => {

        let key = `${DadosFactory._indexAcompanhantes}-${email}`

        return DadosFactory.toArray(DadosFactory.getItem(key));
    },

    /** Retorna os dados separados dos acompanhantes */
    getDataOptionsAcompanhantes: (email) => {
        let acompanhantes = DadosFactory.getDadosAcompanhantes(email);
    ...
    loadAcompanhantes: (email) => {

        let $body = $('body');
        // Cria as listas de ajuda
        let acompanhantes = window.DadosFactory.getDataOptionsAcompanhantes(email);
